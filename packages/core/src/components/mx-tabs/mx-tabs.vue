<template>
	<view class="mx-tabs" :class="customClass" :style="tabsStyle">
		<scroll-view scroll-x :show-scrollbar="false" class="mx-tabs__nav" :class="{ 'mx-tabs__nav--sticky': sticky }" :scroll-with-animation="true" :scroll-left="scrollLeft">
			<view class="mx-tabs__wrap">
				<mx-tab v-for="(pane, index) in panes" :key="pane.name" :title="pane.title" :name="String(pane.name)" :disabled="pane.disabled" @click="clickTab(pane, index)" />
