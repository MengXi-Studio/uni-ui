<template>
	<view class="mx-loading" :class="{ 'mx-loading--vertical': vertical, customClass }" :style="customStyle">
		<!-- circular: 环形旋转 -->
		<view v-if="type === 'circular'" class="mx-loading__spinner mx-loading__spinner--circular" :style="{ color, width: sizeUnit, height: sizeUnit }" :aria-label="text || 'loading'">
		...
		<!-- spinner: 旋转圆环 -->
		<view v-else class="mx-loading__spinner mx-loading__spinner--spinner" :style="{ width: sizeUnit, height: sizeUnit }" :aria-label="text || 'loading'">
			<view v-for="i in 12" :key="i" class="mx-loading__spinner-item" :style="spinnerItemStyle(i)"></view>
		</view>
