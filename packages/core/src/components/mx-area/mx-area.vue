      ...
      <view class="mx-area__columns">
        ...
        >
          <picker-view-column
            ...
            :key="ci"
            class="mx-area__column"
          >
            <view
              v-for="(option, oi) in column"
              :key="oi"
              class="mx-area__option"
              :class="{ 'mx-area__option--selected': oi === indexes[ci] }"
            >
...
const visibleColumns = computed<PickOption[][]>(() => {
  ...
  while (
    lvl < colNum.value - 1 &&
    indexes.value[lvl] !== undefined &&
    cur &&
    cur[indexes.value[lvl]]?.children?.length
  ) {
