<template>
	<view class="mx-floating-panel" :class="customClass" :style="panelStyle">
		<!-- 拖拽把手 -->
		<view class="mx-floating-panel__handle-bar" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd" @touchcancel="onTouchEnd">
...
const anchorList = computed<number[]>(() => {
	const list: number[] = Array.isArray(props.anchors) && props.anchors.length ? (props.anchors as number[]).map(Number) : []
