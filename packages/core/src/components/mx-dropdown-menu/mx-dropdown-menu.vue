<template>
	<view class="mx-dropdown-menu" :class="customClass" :style="rootStyle">
		...
		<!-- 遮罩 -->
		<mx-overlay v-if="overlay && activeIndex >= 0" class="mx-dropdown-menu__overlay" :show="true" :z-index="zIndex" :duration="durationNumber" :custom-style="overlayStyle" @click="onClickOverlay" />

		<!-- 弹出选项面板 -->
		<view
			v-if="activeIndex >= 0"
			class="mx-dropdown-menu__panel"
			:class="[activeItem && activeItem.direction ? `mx-dropdown-menu__panel--${activeItem.direction}` : '', 'mx-dropdown-menu__panel--enter']"
			:style="panelStyle">
