<template>
	<view class="mx-address-edit" :class="customClass" :style="customStyle">
		...
		<view class="mx-address-edit__cell">
			...
			<input class="mx-address-edit__input" :value="form.name" :placeholder="namePlaceholder" maxlength="20" @input="onField('name', $event)" />
		</view>

		<!-- 电话 -->
		<view class="mx-address-edit__cell">
			<text class="mx-address-edit__label">电话</text>
			<input class="mx-address-edit__input" type="tel" :value="form.tel" :placeholder="telPlaceholder" maxlength="15" @input="onField('tel', $event)" />
		</view>
		...
		<view v-if="showDetail" class="mx-address-edit__cell mx-address-edit__cell--textarea">
			...
			<textarea class="mx-address-edit__textarea" :value="form.addressDetail" :placeholder="detailPlaceholder" :maxlength="detailMaxlength" auto-height @input="onField('addressDetail', $event)" />
		</view>

		<!-- 邮政编码 -->
		<view v-if="showPostal" class="mx-address-edit__cell">
			<text class="mx-address-edit__label">邮政编码</text>
			<input class="mx-address-edit__input" type="number" :value="form.postalCode || ''" :placeholder="postalPlaceholder" maxlength="6" @input="onField('postalCode', $event)" />
		</view>
		...
		<!-- 省市区选择 -->
		<mx-picker :show="showArea" :columns="areaColumns" :title="areaTitle" confirm-button-text="确认" cancel-button-text="取消" @update:show="showArea = $event" @confirm="onAreaConfirm" />
	</view>
</template>
