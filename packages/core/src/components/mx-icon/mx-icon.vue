<template>
	<view class="mx-icon" :class="[{ 'mx-icon--dot': dot, 'mx-icon__badge--active': !!badge }, customClass]" :style="iconStyle">
		<!-- 图片图标 -->
		<image v-if="imageSrc" class="mx-icon__image" :src="imageSrc" :style="{ width: iconSizeUnit, height: iconSizeUnit }" mode="aspectFit" />
		<!-- 字体/符号图标 -->
		<text v-else class="mx-icon__name" :class="name ? classPrefix + ' ' + classPrefix + '-' + name : ''" :style="{ color, fontSize: iconSizeUnit }">
			{{ glyph }}
		</text>
