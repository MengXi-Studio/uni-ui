...
const barStyle = computed(() => {
	const fill = props.vertical ? { bottom: '0', height: `${percent.value}%` } : { top: '0', width: `${percent.value}%` }
	return {
...
const thumbStyle = computed(() => {
	const pos = props.vertical ? { bottom: `calc(${percent.value}% - ${buttonSize.value} / 2)`, top: 'auto', left: '50%' } : { left: `calc(${percent.value}% - ${buttonSize.value} / 2)`, top: '50%' }
	return {
	...
	return new Promise(resolve => {
		...
		query
			...
			.boundingClientRect((rect: any) => {
				...
				range = props.vertical ? { start: rect.top, size: rect.height || 0 } : { start: rect.left, size: rect.width || 0 }
				resolve()
			})
			.exec()
	})
