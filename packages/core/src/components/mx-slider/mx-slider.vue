...
const trackStyle = computed(() => {
  const size = props.vertical
    ? { bottom: '0', top: '0', width: addUnit(props.barHeight) }
    : { left: '0', right: '0', height: addUnit(props.barHeight) }
  return {
...
const updateValue = (event: any) => {
  ...
  const ratio =
    step.value === 1 ? raw : min.value + Math.round((raw - min.value) / step.value) * step.value
