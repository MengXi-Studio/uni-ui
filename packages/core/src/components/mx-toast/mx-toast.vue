<template>
	...
	<view v-if="transition.render" class="mx-toast" :class="[`mx-toast--${position}`, midTransition, transition.transitionClass, customClass]" :style="toastStyle">
		<!-- loading 类型 -->
		...
		<view v-else-if="currentIcon" class="mx-toast__icon" :style="{ width: iconSizeUnit, height: iconSizeUnit }">
			<image v-if="isImageIcon" class="mx-toast__icon-img" :src="currentIcon" mode="aspectFit" :style="{ width: iconSizeUnit, height: iconSizeUnit }" />
