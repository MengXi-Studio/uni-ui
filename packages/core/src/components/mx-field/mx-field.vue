<template>
	<view class="mx-field" :class="[`mx-field--label-${labelAlign}`, { 'mx-field--borderless': !border, 'mx-field--large': size === 'large' }, customClass]" :style="customStyle" @click="onClick">
		...
		<view class="mx-field__body">
			...
			<input
				...
				@blur="onBlur"
				@click="onClickInput" />
			<view v-if="showWordLimit && maxlength > 0" class="mx-field__word-limit"> {{ String(modelValue || '').length }}/{{ maxlength }} </view>
			<slot name="extra" />
		</view>
...
const showRightArea = computed(() => showClear.value || props.rightIconName || props.isLink || !!slots['right-icon'])

const showClear = computed(() => props.clearable && !props.disabled && !props.readonly && String(props.modelValue || '') !== '')
