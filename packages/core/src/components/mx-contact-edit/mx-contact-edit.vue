<template>
	<view class="mx-contact-edit" :class="customClass" :style="customStyle">
		<mx-cell-group>
			<mx-field v-model="contact.name" label="姓名" :maxlength="nameMaxLengthNum" placeholder="请输入姓名" :required="true" />
			<mx-field v-model="contact.tel" label="电话" type="tel" :maxlength="telMaxLengthNum" placeholder="请输入手机号" :required="true" />
			<mx-field v-if="showContactType" label="类型" :border="false">
				<view class="mx-contact-edit__types">
					<view v-for="opt in typeOptions" :key="opt.value" class="mx-contact-edit__type" @click="onPickType(opt.value)">
