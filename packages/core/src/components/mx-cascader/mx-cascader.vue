<template>
	<mx-popup :show="show" position="bottom" :round="true" :z-index="zIndex" @update:show="onUpdateShow">
		<view class="mx-cascader" :class="customClass" :style="customStyle">
			...
			<!-- tabs (variant=tabs) -->
			<scroll-view v-if="variant === 'tabs'" class="mx-cascader__tabs" scroll-x :show-scrollbar="false">
				<view class="mx-cascader__tab-wrap">
					<view v-for="(tab, ti) in tabs" :key="ti" class="mx-cascader__tab" :class="{ 'mx-cascader__tab--active': ti === activeTab }" @click="onSwitchTab(ti)">
						<text class="mx-cascader__tab-text">{{ tab }}</text>
					</view>
					<view class="mx-cascader__tab" :class="[activeTab === tabs.length ? 'mx-cascader__tab--active' : '']">
			...
			<scroll-view v-else class="mx-cascader__crumbs" scroll-x :show-scrollbar="false">
				<view class="mx-cascader__crumbs-wrap">
					<view v-for="(c, ci) in crumbTexts" :key="ci" class="mx-cascader__crumb" @click="onSwitchCrumb(ci)">
			...
			<scroll-view scroll-y class="mx-cascader__list">
				<view v-for="(option, oi) in currentOptions" :key="oi" class="mx-cascader__option" :class="{ 'mx-cascader__option--selected': oi === selectedIndex }" @click="onSelect(option, oi)">
			...
			<view class="mx-cascader__footer">
				<mx-button size="small" plain custom-class="mx-cascader__step" @click="onPrev"> 上一步 </mx-button>
				<mx-button size="small" type="primary" custom-class="mx-cascader__next" :disabled="!currentSelection" @click="onNext"> 下一步 </mx-button>
			</view>
		</view>
	</mx-popup>
</template>
...
/** 当前选中 option */
const currentSelection = computed<CascaderOption | null>(() => currentOptions.value[selectedIndex.value] || null)

const hasChildren = (option: CascaderOption) => Array.isArray(option?.[FN.value.children]) && option[FN.value.children].length > 0
