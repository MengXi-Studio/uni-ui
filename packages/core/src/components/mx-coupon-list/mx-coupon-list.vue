    ...
    <view class="mx-coupon-list__footer">
      <view v-if="showExchangeBar" class="mx-coupon-list__exchange">
        <input
          ...
          class="mx-coupon-list__exchange-input"
...
const availableCoupons = computed(() =>
  (props.coupons as CouponInfo[]).filter((c) => c.available !== false)
)
...
const onClickCoupon = (coupon: CouponInfo) => {
  ...
  const next = isChosen(coupon)
    ? chosenIds.value.filter((v) => String(v) !== String(id))
    : [...chosenIds.value, id]
