<template>
	<view v-if="transition.render" class="mx-image-preview" :class="[transition.transitionClass, customClass]" :style="rootStyle">
		<!-- 遮罩渐变黑 -->
		...
		<swiper class="mx-image-preview__swiper" :current="current" @change="onChange" @touchmove.stop>
			<swiper-item v-for="(img, i) in images" :key="i" class="mx-image-preview__item">
				<view class="mx-image-preview__image-wrap" @click="onClickImage">
					<image class="mx-image-preview__image" :src="img" mode="aspectFit" :style="zoomStyle" @click.stop />
				</view>
			</swiper-item>
		</swiper>
	</view>
