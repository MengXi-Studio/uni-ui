<template>
	<mx-overlay v-if="overlay" :show="overlayShow" :duration="Number(duration) || 300" :z-index="zIndex" :custom-style="overlayStyle" :custom-class="overlayClass" @click="onClickOverlay" />
	...
	<view
		...
		<!-- 关闭图标 -->
		<text v-if="closeable && position !== 'center'" class="mx-popup__close-icon" :class="[`mx-popup__close-icon--${closeIconPosition}`]" @click.stop="onClickCloseIcon"> × </text>
	</view>
</template>
