<template>
	<mx-popup :show="show" position="bottom" :round="round" :z-index="zIndex" @update:show="onUpdateShow">
		<view class="mx-calendar" :class="customClass" :style="customStyle">
			...
			<view class="mx-calendar__nav">
				<text class="mx-calendar__nav-arrow" :class="{ 'mx-calendar__nav-arrow--disabled': !canGoPrev }" @click="prevMonth"> ‹ </text>
				<text class="mx-calendar__nav-title">{{ viewYear }}年{{ viewMonth + 1 }}月</text>
				<text class="mx-calendar__nav-arrow" :class="{ 'mx-calendar__nav-arrow--disabled': !canGoNext }" @click="nextMonth"> › </text>
			</view>
...
/** 初始选中 */
const initSelected = () => {
	const src = props.modelValue != null && (props.modelValue as any).toString() !== '' ? props.modelValue : props.defaultDate
	if (src == null) {
		selectedDates.value = []
		return
	}
...
watch(
	...
	val => {
		if (props.type === 'single') {
			...
			selectedDates.value = Array.isArray(val) ? (val as number[]).map(t => startOfDay(Number(t))).sort((a, b) => a - b) : []
		}
	}
)
