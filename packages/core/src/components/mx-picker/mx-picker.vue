	...
	<mx-popup :show="show" position="bottom" :round="true" :z-index="zIndex" @update:show="onUpdateShow">
		<view class="mx-picker" :class="customClass" :style="customStyle">
			...
			<view class="mx-picker__columns">
				<picker-view class="mx-picker__view" :value="viewValue" @change="onChange" @touchmove.stop.prevent>
					<picker-view-column v-for="(column, ci) in visibleColumns" :key="ci" class="mx-picker__column">
						<view v-for="(option, oi) in column" :key="oi" class="mx-picker__option" :class="{ 'mx-picker__option--selected': oi === selectedIndexes[ci] }">
...
const independent = computed(() => Array.isArray(props.columns) && props.columns.length > 0 && Array.isArray(props.columns[0]))
...
/** 独立多列: 每列一个选项数组 */
const independentColumns = computed(() => (props.columns as any[]).map(col => toOptions(Array.isArray(col) ? col : [])))
...
const visibleColumns = computed<PickerOption[][]>(() => {
	...
	cols.push(cur)
	let lvl = 0
	while (selectedIndexes.value[lvl] !== undefined && cur && cur[selectedIndexes.value[lvl]]?.children?.length) {
...
watch(
	...
	() => {
		...
		if (selectedIndexes.value.length < len) {
			selectedIndexes.value = selectedIndexes.value.concat(Array(len - selectedIndexes.value.length).fill(0))
		}
	},
	{ immediate: true }
)
