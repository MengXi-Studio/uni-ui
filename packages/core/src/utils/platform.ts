...
export const platform = {
	...
	/** 小程序 */
	isMp: process.env.UNI_PLATFORM === 'mp-weixin' || process.env.UNI_PLATFORM === 'mp-alipay' || process.env.UNI_PLATFORM === 'mp-baidu' || process.env.UNI_PLATFORM === 'mp-toutiao',
 ...
 * 根据平台执行不同的回调
 */
export function onPlatform<T>(callbacks: { h5?: () => T; mp?: () => T; app?: () => T; default?: () => T }): T | undefined {
 ...
 * 安全调用 API，失败时不报错
 */
export function safeCallAPI<T extends keyof typeof uni>(apiName: T, ...args: Parameters<(typeof uni)[T]>): Promise<ReturnType<(typeof uni)[T]>> {
